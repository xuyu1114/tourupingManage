...
class Infomation extends Api{
    ...
    public function getTotalAndActive(){
        try {
            ...
            $alive_shop = Db::table("trp_collect")
                ->group("shop_name")
                ->where("createtime","between",$data_where)
                ->column("shop_name");
    ...
    public function getHerbicidePurchasing(){
        try{
            ...
            $last_month_today = date("Y-m-d 00:00:00",strtotime("-30 day"));
            $today = date("Y-m-d 23:59:59",time());
            $sql = "select shop_name,sum(if(a.unit='g'||a.unit='ml'||a.unit='克'||a.unit='毫升',(a.attr*a.num/1000),a.attr*a.num)) total from trp_goods_purchase a
                where category = '除草剂'  and a.createtime BETWEEN '{$last_month_today}' and '{$today}' 
    ...
    public function getSaleInfo(){
        try{
            ...
            foreach ($arr_category as $key => $val){
                $sql = "select b.current_goods_name as goods_name,SUM(a.weight) as weight from trp_collect a
                        LEFT JOIN trp_goods_contrast b on a.brand_name = b.goods_name
                        where a.category in {$val} and a.createtime BETWEEN '{$last_month_today}' and '{$today}' and b.current_goods_name is not null
    ...
    public function getTwoMinus(){
        try{
            ...
            $cgl_weight_last_year = Db::table("trp_collect")
                ->where("createtime",">",$last_year)
                ->where("createtime","<",$this_year)
            ...
            $cgl_weight_this_year = Db::table("trp_collect")
                ...
                ->sum("weight");
            $ret_data["cgl"] = array(
                date("Y",strtotime($last_year)) => $cgl_weight_last_year,
                date("Y",strtotime($this_year)) => $cgl_weight_this_year,
            );
            ...
            $hf_weight_this_year = Db::table("trp_collect")
                ...
                ->sum("weight");
            $ret_data["hf"] = array(
                date("Y",strtotime($last_year)) => $hf_weight_last_year,
                date("Y",strtotime($this_year)) => $hf_weight_this_year,
            );
            $this->success("success",$ret_data,0);
        }catch (Exception $e){
            $this->error("数据查询失败",[],-1);
